...
if ( Sensei()->course->is_user_enrolled( get_the_ID(), get_current_user_id() ) ) :
	...
	if ( ! empty( $lesson_url ) ) :
		echo '<div class="sensei-block-wrapper"><a href="' . esc_url( $lesson_url ) . '" class="wp-block-button__link">' . esc_html__( 'Start course', 'beflex-child' ) . '</a></div>';
	endif;
else :
	$content = '<button class="wp-block-button__link">' . esc_html__( 'Take course', 'beflex-child' ) . '</button>';
