...
function generateVariants(value) {
  ...
  // Ajouter le préfixe "bfs-"
  const bfLowerCase = `bfs-${lowerCase}`;
  const bfUpperCase = `BFS_${upperCase}`;
  const bfSnakeCase = `bfs_${snakeCase}`;
...
function generateBlockStructure(destinationFolder, value) {
  const blockFolder = path.join(destinationFolder, `bfs-${value}`);
